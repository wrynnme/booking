...
<?php
if (isset($_GET['del'])) {
	$con->query("DELETE FROM `interpreter` WHERE `itp_id` = '" . $_GET['del'] . "'");
...
if (isset($_GET['del2'])) {
	...
	$headmessages = "Requester name : <b>$to[bk_name]</b> <br> Department : <b>$to[bk_dept]</b> <br> Telephone number : <b>$to[bk_tel]</b> <br> Objective : <b>$to[bk_objective]</b> <br> Email : <b>$to[bk_email]</b> <br> Time start : <b>$to[bktime_start]</b> <br> Time end : <b>$edate</b>";
...
<head>
	<meta charset="utf-8">
	<title></title>
	<link rel='stylesheet' href='/booking/fullcalendar/fullcalendar.css' />
	<script src='/booking/fullcalendar/lib/jquery.min.js'></script>
	<script src='/booking/fullcalendar/lib/moment.min.js'></script>
	<script src='/booking/fullcalendar/fullcalendar.js'></script>
	...
	<div class="container">
		<form action="" method="get" name="fs" id="fs">
			...
			<div class="form-group">
				<div class="row row justify-content-between">
					...
					<div class="col-md-5">
						<a class="btn btn-outline-primary btn-block" href="addinterpreter.php" class="">Add Interpreter</a>
					</div>
				</div>
				<br>
				...
				<tbody>
					<?php while ($r = $sql->fetch_array(MYSQLI_ASSOC)) { ?>
						<tr class="text-center">
							...
							<td>
								<font>
									<a href="edit_interpreter.php?itp_id=<?php echo $r['itp_id'] ?>"> <img src="/booking/images/edit.png" width="25px" style="margin-bottom:3px;"> </a> /
									<a href="?del=<?php echo $r['itp_id']; ?>" onClick="return confirm('Are your sure to delete?')"> <img src="/booking/images/delete.png" width="25px" style="margin-bottom:3px;"> </a>
								</font>
							</td>
						</tr>
					<?php } ?>
		...
		<form id="ap">
			...
			<table class="table table-hover">
				...
				$x = 1;
				$row = 10;
				$sql = $con->query("SELECT * FROM bk_interpreter,employee WHERE employee.emp_id = bk_interpreter.bkemp_id AND bk_interpreter.bk_factory = 1  AND bk_interpreter.bk_mgr_ap = 1  AND bk_interpreter.bktime_start >= '$now' ORDER BY bk_interpreter.itp_id,bk_interpreter.bkitp_id,bk_interpreter.bktime_start ASC  LIMIT $start,$row");
