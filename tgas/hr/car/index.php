...
<?php
if (isset($_GET['del'])) {
	$con->query("DELETE FROM `car` WHERE `car_id` = '" . $_GET['del'] . "'");
	...
	<div class="container">
		<form action="" method="get" name="fs" id="fs">
			...
			<div class="form-group">
				<div class="row row justify-content-between">
					...
					<div class="col-md-5">
						<a class="btn btn-outline-primary btn-block" href="addcar.php" class="">Add Car</a>
					</div>
				</div>
				<br>
				...
				<tbody>
					<?php while ($r = $sql->fetch_array(MYSQLI_ASSOC)) { ?>
						<tr class="text-center">
							<td><?php echo $x;
									$x++; ?></td>
							<td><a href="/booking/tgas/viewcar.php?car_id=<?= $r['car_id'] ?>"><?php echo $r['license_plate']; ?></a></td>
							<td><a href="/booking/tgas/viewcar.php?car_id=<?= $r['car_id'] ?>"><?php echo $r['driver_name'] ?></a></td>
							<td><a href="/booking/tgas/viewcar.php?car_id=<?= $r['car_id'] ?>"><?php echo $r['driver_tel'] ?></a></td>
							<td><a href="/booking/tgas/viewcar.php?car_id=<?= $r['car_id'] ?>"><?php echo $r['car_brand'] ?></a></td>
							<td><a href="/booking/tgas/viewcar.php?car_id=<?= $r['car_id'] ?>"><input class="form-control" type="color" value="<?php echo $r['car_color'] ?>" id="inputcar_color" name="inputcar_color" style="height: 38px;" disabled></a></td>
							<td>
							...
							<td>
								<font>
									<a href="edit_car.php?car_id=<?= $r['car_id'] ?>"> <img src="/booking/images/edit.png" width="25px" style="margin-bottom:3px;"> </a> /
									<a href="?del=<?= $r['car_id']; ?>" onClick="return confirm('Are your sure to delete?')"> <img src="/booking/images/delete.png" width="25px" style="margin-bottom:3px;"> </a>
								</font>
							</td>
						</tr>
					<?php } ?>
		...
		<form id="ap">
			...
			<table class="table table-hover">
				...
				$x = 1;
				$row = 10;
				$sql = $con->query("SELECT * FROM bk_car,employee WHERE employee.emp_id = bk_car.bkemp_id AND bk_car.bk_factory = 1  AND bk_car.bk_mgr_ap = '1' AND bk_car.bktime_start >= '$now' ORDER BY bk_car.car_id,bk_car.bkcar_id,bk_car.bktime_start ASC  LIMIT $start,$row");
					...
					<?php while ($r = $sql->fetch_array(MYSQLI_ASSOC)) {
						...
						if ($r['bk_mgr_ap'] == '1') {
							...
							<?php } elseif ($r['bk_mgr_ap'] == '2') { ?>
								...
								<td class="text-center">
									<a href="./viewbookingcar.php?bkcar_id=<?php echo $r['bkcar_id'] ?>">
										<?= date_format($sdate, 'l d F Y H:i') ?>
									</a>
								</td>
								<td class="text-center">
									<a href="./viewbookingcar.php?bkcar_id=<?php echo $r['bkcar_id'] ?>">
										<?= date_format($edate, 'l d F Y H:i') ?>
									</a>
								</td>
								<td class="text-center">
									<a href="./viewbookingcar.php?bkcar_id=<?php echo $r['bkcar_id'] ?>">
								...
								<td>
									<font>
										<a href="?del2=<?= $r['bkcar_id']; ?>" onClick="return confirm('Are your sure to delete?')"> <img src="/booking/images/delete.png" width="25px" style="margin-bottom:3px;"> </a>
									</font>
								</td>
								</tr>
							<?php } ?>
