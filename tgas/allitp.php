...
if (isset($_GET['select'])) {
	if ($_GET['select'] == 'year') {
		$tt = $con->query("SELECT * FROM bk_interpreter,employee WHERE bk_interpreter.bk_factory = 1 AND bk_interpreter.bkemp_id = employee.emp_id AND YEAR(bktime_end) = '$nowyear' AND bk_interpreter.bk_mgr_ap = 1 ORDER BY `bk_interpreter`.`bktime_start`, `bk_interpreter`.`bktime_end` DESC");
	} elseif ($_GET['select'] == 'month') {
		$tt = $con->query("SELECT * FROM bk_interpreter,employee WHERE bk_interpreter.bk_factory = 1 AND bk_interpreter.bkemp_id = employee.emp_id AND YEAR(bktime_end) = '$nowyear' AND MONTH(bktime_end) = '$nowmonth' AND bk_interpreter.bk_mgr_ap = 1 ORDER BY `bk_interpreter`.`bktime_start`, `bk_interpreter`.`bktime_end` DESC");
	} elseif ($_GET['select'] == 'day') {
		$tt = $con->query("SELECT * FROM bk_interpreter,employee WHERE bk_interpreter.bk_factory = 1 AND bk_interpreter.bkemp_id = employee.emp_id AND YEAR(bktime_end) = '$nowyear' AND MONTH(bktime_end) = '$nowmonth' AND DAY(bktime_end) = '$nowday' AND bk_interpreter.bk_mgr_ap = 1 ORDER BY `bk_interpreter`.`bktime_start`, `bk_interpreter`.`bktime_end` DESC");
	}
} else {
	$tt = $con->query("SELECT * FROM bk_interpreter,employee WHERE bk_interpreter.bk_factory = 1 AND bk_interpreter.bkemp_id = employee.emp_id AND YEAR(bktime_end) = '$nowyear' AND MONTH(bktime_end) = '$nowmonth' AND bk_interpreter.bk_mgr_ap = 1 ORDER BY `bk_interpreter`.`bktime_start`, `bk_interpreter`.`bktime_end` DESC");
}
