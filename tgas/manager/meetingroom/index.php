	...
	<div class="container">
		...
		<form id="ap">
			...
			<table class="table table-hover">
				...
				$x = 1;
				$row = 10;
				$sql = $con->query("SELECT * FROM bk_mtr,meeting_room,employee WHERE bk_mtr.mtr_id = meeting_room.mtr_id AND bk_mtr.bkemp_id = employee.emp_id AND bk_mtr.bk_factory = 1 AND bk_mtr.bktime_end >= '$now' AND employee.emp_dept = (SELECT employee.emp_dept FROM employee WHERE emp_id = '$_SESSION[emp_id]') ORDER BY bk_mtr.bkmtr_id,meeting_room.mtr_number,bk_mtr.bktime_start ASC  LIMIT $start,$row");
