...
include './../../header.php';
$qdet = $con->query("SELECT * FROM bk_mtr WHERE bkmtr_id = '$_GET[bkmtr_id]'");
$det = $qdet->num_rows;
if ($det < 1) {
	ao("This booked meeting room has deleted", '/booking/tgas/manager/meetingroom');
}
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
	<?php
	$qr = $con->query("SELECT * FROM bk_mtr,employee,meeting_room WHERE meeting_room.mtr_id = bk_mtr.mtr_id AND employee.emp_id = bk_mtr.bkemp_id AND bk_mtr.bk_factory = 1 AND bk_mtr.bkmtr_id = '$_GET[bkmtr_id]'");
	...
	if (isset($_POST['approved'])) {
		if ($r['bk_mgr_ap'] == 1) {
			ao("Not update a booked meeting room.", '/booking/TGAS/manager/meetingroom/');
		} else {
			$bk_mgr_ap = 1;
			$sdate = $r['bktime_start'];
			$edate = $r['bktime_end'];
			$con->query("UPDATE `bk_mtr` SET `bk_mgr_ap`= '$bk_mgr_ap' , `bk_hr_ap` = NULL WHERE `bkmtr_id` = '$_GET[bkmtr_id]'");
			ao("Approve a booked meeting room.", '/booking/TGAS/manager/meetingroom/');
	...
	if (isset($_POST['disapproves'])) {
		if ($r['bk_mgr_ap'] == 2) {
			...
			$sdate = $r['bktime_start'];
			$edate = $r['bktime_end'];
			$con->query("UPDATE `bk_mtr` SET `bk_mgr_ap`= '$bk_mgr_ap' , `bk_hr_ap` = '2' WHERE `bkmtr_id` = '$_GET[bkmtr_id]'");
			ao("Disapproves a booked meeting room.", '/booking/TGAS/manager/meetingroom/');
			...
			$strSubject   = "=?UTF-8?B?" . base64_encode("Manager has disapproves a booked meeting room.") . "?=";
			$Subject      = "Manager has disapproves a booked meeting room.";
			$strHeader   .= "MIME-Version: 1.0' . \r\n";
			$strHeader   .= "Content-type: text/html; charset=utf-8\r\n";
			$strHeader  .= "From: Booking Management System <$_SESSION[emp_email]>\r\n";
			$strHeader   .= "Cc: User Requester <$e[emp_email]>\r\n";
			$headmessages = "Meeting room : <b>$r[mtr_number]</b> <br> Requester name : <b>$r[bk_name]</b> <br> Department : <b>$r[bk_dept]</b> <br> Telephone number : <b>$r[bk_tel]</b> <br> Email : <b>$r[bk_email]</b> <br> Time start : <b>$r[bktime_start]</b> <br> Time end : <b>$edate</b> <br> Objective : <b>$r[bk_objective]</b> <br><br> <a target='_blank' href='http://192.168.222.223/booking/tgas/user/meetingroom/viewbookingmtr.php?bkmtr_id=$lii'>Click here to check a booked</a> <br>";
			include './../../mail.php';
			$flgSend = mail($strTo, $strSubject, $strMessage, $strHeader);
			if ($flgSend) {
				al_g("Send email successfully");
