...
<?php
if (isset($_GET['del2'])) {
	$qto = $con->query("SELECT * FROM `bk_car` WHERE `bkcar_id` = '" . $_GET['del2'] . "' ");
	...
	<div class="container"><br>
		...
		<form id="ap">
			...
			<table class="table table-hover">
				...
				$x = 1;
				$row = 10;
				$sql = $con->query("SELECT * FROM bk_car,employee WHERE employee.emp_id = bk_car.bkemp_id AND bk_car.bk_factory = 1 AND bk_car.bkemp_id = '$_SESSION[emp_id]' AND bk_car.bktime_end >= '$now' AND employee.emp_dept = (SELECT employee.emp_dept FROM employee WHERE emp_id = '$_SESSION[emp_id]') ORDER BY bk_car.bkcar_id DESC LIMIT $start,$row");
