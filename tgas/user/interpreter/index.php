...
<?php
if (isset($_GET['del2'])) {
	$qto = $con->query("SELECT * FROM `bk_interpreter` WHERE `bkitp_id` = '" . $_GET['del2'] . "' ");
...
<head>
	...
	<script language="JavaScript">
		function fncHideap() { //Command to Hide
			...
			document.getElementById('btnShowap').setAttribute("value", "Show your booking !");
		...
		function fncShowap() { //Command to Show
			...
			document.getElementById('btnShowap').setAttribute("value", "Hide your booking !");
	...
	<div class="container">
		<form action="" method="get" name="fs" id="fs">
			...
			<div class="form-group">
				...
				<div class="row justify-content-end">
					<div class="col-md-3 ">
						<input class="btn btn-outline-secondary btn-block" name="btnShowap" id="btnShowap" type="button" value="Hide your booking !" onClick="JavaScript:fncHideap();">
					</div>
					<div class="col-md-3">
		...
		<form id="ap">
			...
			<table class="table table-hover">
				...
				$x = 1;
				$row = 10;
				$sql = $con->query("SELECT * FROM bk_interpreter,employee WHERE employee.emp_id = bk_interpreter.bkemp_id AND bk_interpreter.bk_factory = 1 AND bk_interpreter.bkemp_id = $_SESSION[emp_id] AND bk_interpreter.bkemp_id = $_SESSION[emp_id] AND bk_interpreter.bktime_end >= '$now' AND employee.emp_dept = (SELECT employee.emp_dept FROM employee WHERE emp_id = '$_SESSION[emp_id]') ORDER BY bk_interpreter.bktime_start,bk_interpreter.bkitp_id,bk_interpreter.itp_id ASC LIMIT $start,$row");
