...
if (isset($_GET['select'])) {
	if ($_GET['select'] == 'year') {
		$tt = $con->query("SELECT * FROM bk_mtr,employee,meeting_room WHERE bk_mtr.bk_factory = 1 AND bk_mtr.mtr_id = meeting_room.mtr_id AND bk_mtr.bkemp_id = employee.emp_id AND YEAR(bktime_end) = '$nowyear' AND bk_mtr.bk_mgr_ap = 1 ORDER BY `bk_mtr`.`bktime_start`, `bk_mtr`.`bktime_end` DESC");
	} elseif ($_GET['select'] == 'month') {
		$tt = $con->query("SELECT * FROM bk_mtr,employee,meeting_room WHERE bk_mtr.bk_factory = 1 AND bk_mtr.mtr_id = meeting_room.mtr_id AND bk_mtr.bkemp_id = employee.emp_id AND YEAR(bktime_end) = '$nowyear' AND MONTH(bktime_end) = '$nowmonth' AND bk_mtr.bk_mgr_ap = 1 ORDER BY `bk_mtr`.`bktime_start`, `bk_mtr`.`bktime_end` DESC");
	} elseif ($_GET['select'] == 'day') {
		$tt = $con->query("SELECT * FROM bk_mtr,employee,meeting_room WHERE bk_mtr.bk_factory = 1 AND bk_mtr.mtr_id = meeting_room.mtr_id AND bk_mtr.bkemp_id = employee.emp_id AND YEAR(bktime_end) = '$nowyear' AND MONTH(bktime_end) = '$nowmonth' AND DAY(bktime_end) = '$nowday' AND bk_mtr.bk_mgr_ap = 1 ORDER BY `bk_mtr`.`bktime_start`, `bk_mtr`.`bktime_end` DESC");
	}
} else {
	$tt = $con->query("SELECT * FROM bk_mtr,employee,meeting_room WHERE bk_mtr.bk_factory = 1 AND bk_mtr.mtr_id = meeting_room.mtr_id AND bk_mtr.bkemp_id = employee.emp_id AND YEAR(bktime_end) = '$nowyear' AND MONTH(bktime_end) = '$nowmonth' AND bk_mtr.bk_mgr_ap = 1 ORDER BY `bk_mtr`.`bktime_start`, `bk_mtr`.`bktime_end` DESC");
}
		...
		while ($r = $tt->fetch_array(MYSQLI_ASSOC)) {
			...
			if ($r['bk_mgr_ap'] == '1') {
				...
				<?php } elseif ($r['bk_mgr_ap'] == '2') { ?>
					...
					<?php } ?>
					<td class="text-center"><a href="/booking/tgas/viewmtr.php?mtr_id=<?php echo $r['mtr_id'] ?>"><?php echo $r['mtr_number'] ?></a></td>
					<td class="text-center"><?php echo $r['bk_name'] ?></td>
					<td class="text-center"><?php echo $r['bk_tel'] ?></td>
