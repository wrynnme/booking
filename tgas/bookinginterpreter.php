...
<body>
	...
	if (isset($_POST['done'])) {
		...
		if ($bk_name == "" || $sdate == "" || $edate == "" || $bk_email == "" || $bk_tel == "" || $bk_dept == "") {
			...
			$sql = $con->query("INSERT INTO `bk_interpreter` VALUES (NULL, '$bk_name', '$bk_dept', '$bk_tel', '$bk_email', '$bk_objective', '$sdate','$edate', NULL, '1', $bkemp_id, NULL, NULL);");
			if ($sql) {
				$qe = $con->query("SELECT * FROM `employee` WHERE emp_id = $_SESSION[emp_id]");
				$qrr = $con->query("SELECT interpreter.itp_name FROM `interpreter` WHERE itp_id = $itp_id");
				...
				$qlink = $con->query("SELECT * FROM bk_interpreter WHERE bk_name = '$bk_name' AND bk_tel = '$bk_tel' AND bk_email = '$bk_email' AND bk_dept = '$bk_dept' AND bktime_start = '$sdate' AND bktime_end = '$edate' AND bkemp_id = '$_SESSION[emp_id]'");
				...
				$strSubject  = "=?UTF-8?B?" . base64_encode("The requester has booking interpreter.") . "?=";
				$Subject     = "The requester has booking interpreter.";
				$strHeader  .= "MIME-Version: 1.0' . \r\n";
				$strHeader  .= "Content-type: text/html; charset=utf-8\r\n";
				$strHeader  .= "From: Booking Management System <$bk_email>\rReply-To:<$bk_email>\r\n";
				$strHeader  .= "Cc: User Requester<$bk_email>\r\n";

				$headmessages = "Requester name : <b>$bk_name</b> <br> Department : <b>$bk_dept</b> <br> Date Start : <b>$sdate</b> <br> Date End : <b>$edate</b> <br> Telephone number : <b>$bk_tel</b> <br> Email requestor : <b> $bk_email </b> <br> Objective : <b>$bk_objective</b> <br><br> <a target='_blank' href='http://192.168.222.223/booking/tgas/manager/interpreter/viewbookinginterpreter.php?bkitp_id=$lii'>Click here to approval page</a> <br>";
				...
				ao("Booking successfully !", '/booking/tgas/$pt/interpreter/');
			}
		}
	}
	?>
	<div class="container" style="margin-top: 30px;">
		<h3 class="text-center"><b>Form <font color="red">B</font>ooking Interpreter</b></h3>
		<br>
		<div class="container" id="form1">
			<form class="" action="<?php $_SERVER['PHP_SELF']; ?>" method="post" id="">
				...
				<div class="row">
					...
					<div class="col-md-10">
						<textarea name="bk_objective" class="col form-control form-check" id="bk_objective" cols="100" rows="3" maxlength="200"></textarea>
					</div>
				</div>
				<hr />
